"""Compare v8 vs baselines in v8's real-rules env."""
...
from env_v8 import GuandanEnvV8
...
def load_qnet(path, device):
    ...
    qnet = QNet(state_dim=STATE_DIM, action_dim=ACTION_DIM,
                hidden=int(ck.get('args', {}).get('hidden', 1024)),
                num_layers=int(ck.get('args', {}).get('num_layers', 4)),
                belief_dim=int(ck.get('args', {}).get('belief_dim', 0))).to(device)
...
@torch.no_grad()
def pick_action(qnet, state_vec, legal_actions, device, epsilon=0.0):
    if epsilon > 0 and random.random() < epsilon or len(legal_actions) == 1:
        return random.choice(legal_actions)
    s = torch.from_numpy(state_vec).float().to(device).unsqueeze(0).expand(len(legal_actions), -1)
    a = torch.from_numpy(np.stack([encode_action(m) for m in legal_actions])).float().to(device)
    q = qnet(s, a)
    return legal_actions[int(q.argmax().item())]


def play_one(env, net_a, net_b, device, opponent_name, learner_team, seed):
...
def main():
    ...
    print(f"\n=== v8 env baselines ({args.games} games each) ===")
    ...
    if len(nets) >= 2:
        print(f"\n=== v8 env head-to-head ({args.games} games) ===")
