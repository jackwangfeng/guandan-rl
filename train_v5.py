"""v5 training: v3 + diversified opponents (random injection) + optional PIMC search
during learner decisions.

Why search even though PIMC at inference didn't help on v4?
  - Search-augmented behavior changes the *state distribution* visited during self-play.
    Even if argmax(Q) is locally optimal, search can push exploration into regions the
    Q-net under-fit, generating different MC returns to regress against.
  - We keep DMC targets (±1 from terminal). Only the *behavior policy* changes.
"""
...
from vec_collect_v5 import collect_batch
...
def assign_seats(n_envs: int, rule_p: float, random_p: float, league_p: float,
                 league_keys: list, rng: random.Random):
    out = {}
    for i in range(n_envs):
        ...
        if r < rule_p:
            opp = 'rule'
        elif r < rule_p + random_p:
            opp = 'random'
        elif r < rule_p + random_p + league_p and league_keys:
...
def main():
    ...
    p.add_argument('--lr', type=float, default=5e-5)  # lower since warm-started
    p.add_argument('--batch', type=int, default=1024)
    p.add_argument('--train-steps-per-batch', type=int, default=8)
    p.add_argument('--epsilon-start', type=float, default=0.15)  # lower since warm-started
    p.add_argument('--epsilon-end', type=float, default=0.03)
    p.add_argument('--epsilon-decay-eps', type=int, default=30_000)
    p.add_argument('--buffer-cap', type=int, default=600_000)
    # opponent mix
    p.add_argument('--rule-p', type=float, default=0.35)
    p.add_argument('--random-p', type=float, default=0.20)
    p.add_argument('--league-p', type=float, default=0.30)
    # league snapshots
    p.add_argument('--league-add-every', type=int, default=10_000)
    p.add_argument('--league-max', type=int, default=8)
    # search injection (PIMC rollout for learner decisions, with given probability)
    p.add_argument('--search-prob', type=float, default=0.05)
    p.add_argument('--search-n-worlds', type=int, default=4)
    # logging / ckpt
    ...
    p.add_argument('--out', type=str, default='runs/v5')
    ...
    while total_eps < args.total_episodes:
        ...
        seat_assignment = assign_seats(args.n_envs, args.rule_p, args.random_p,
                                       args.league_p, league_keys, rng)
        ...
        results = collect_batch(qnet, device, args.n_envs, eps, mode_fn,
                                ...
                                search_prob=args.search_prob,
                                search_n_worlds=args.search_n_worlds)
