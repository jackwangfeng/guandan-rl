"""v7 training: bigger model + belief head + Match-driven self-play.

Episode = one ROUND (within a multi-round match). Reward per round = ±1 by the
first finisher's team (same as v6). Multi-round provides post-tribute hands and
varying level_rank values across the data distribution.

Belief auxiliary loss forces the trunk to encode opponent-hand inference.
"""
...
from vec_collect_v7 import collect_matches
...
def main():
    ...
    # episode budget — here we count ROUNDS (a match has ~5-15 rounds)
    p.add_argument('--total-rounds', type=int, default=600_000,
                   help='target number of rounds (≈ 75k matches at 8 rounds/match)')
    ...
    p.add_argument('--anchor-ckpt', type=str, default='',
                   help='path to a fixed strong opponent (e.g. runs/v6/latest.pt)')
    p.add_argument('--anchor-p', type=float, default=0.20,
                   help='probability the opponent is the anchor (per env-seat)')
    # league snapshots
    ...
    # collection cadence: how many rounds to gather per outer step
    ...
    p.add_argument('--out', type=str, default='runs/v7')
    ...
    qnet = QNet(hidden=args.hidden, num_layers=args.num_layers,
    ...
    if args.anchor_ckpt and Path(args.anchor_ckpt).exists() and args.anchor_p > 0:
        ...
        anchor = build_from_ckpt_args(ck_a.get('args')).to(device)
        anchor.load_state_dict(ck_a['state_dict'])
        anchor.eval()
    ...
    def add_league_snapshot(rd_marker):
        ...
        snap = QNet(hidden=args.hidden, num_layers=args.num_layers,
                    belief_dim=args.belief_dim).to(device)
    ...
    while total_rounds < args.total_rounds:
        ...
        # Snapshot league keys (exclude anchor — it has its own sampling slot)
